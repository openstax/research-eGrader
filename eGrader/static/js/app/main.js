...
var App = {
  ...
  nextResponse(userId, exerciseId) {
    ...
    let response = this.API.getNextResponse(userId, exerciseId)
      .done(function (r) {
        if (r['success'] == true) {
          if (self.debug == true) {
            self.currentResponse = r['response'];
          }
          self.showResponse(r);
        } else {
  ...
  createDebugListItem(itemName, value){

    let $item = $('<li>' + itemName + ': '+ value + '</li>');
  ...
  renderDebugInfo() {
    let $debugInfo = $('.debug-info');
    let $debugList = $('<ul></ul>').addClass('info');

    $debugList.append(this.createDebugListItem('response_id', this.currentResponse.id))
      .append(this.createDebugListItem('active', this.currentResponse.active))
      .append(this.createDebugListItem('step_id', this.currentResponse.step_id))
      .append(this.createDebugListItem('exercise_id', this.currentResponse.exercise_id));
    $debugInfo.append($debugList);
